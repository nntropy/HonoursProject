#!/usr/bin/env python
# From https://github.com/niallmcl/Deep-Android-Malware-Detection
import sys
import os
import shutil
import datetime
...
# This takes an apk and turned it into opcode.
# The sequence is apk -> smali -> opcode
#
def main(apkdir="D:/5th\Honours\Code\dataset\malware/drebin-3", tmpdir="D:/5th/Honours/Code/Utility/maltmp", opdir="D:/5th\Honours\Code\manidataset/malware", suplib=""):

# Usage <apk file directory> <temp directory> <opseq directory> <include support libraries>

    # Reads the location of apk files that need decoding
    apk_file_directory = apkdir
...
def movefilefromdir(dir, movedir):
    """
    :param dir: directory to move all files from
    :param movedir: directory to move all files to
    :return:
    Moves all AndroidManifest files from one directory to another. Can modify movefile() to make it port any file.
    """
    for name in os.listdir(dir):
        try:
            movefile(dir + "/" + name + "/AndroidManifest.xml", movedir + "/" + name + ".xml")
        except FileNotFoundError:
            pass


def movefile(file, movedfile):
    """
    :param file: file to be moved
    :param movedfile: exact location to be moved
    :return: None

    Moves a file from file to movedfile. Used to get AndroidManifests to a single directory
    """
    import shutil
...
def create_opcode_seq(decoded_dir, opseq_file_directory, apk_hash):
    ...
    # writes the corresponding opcode sequence to a .opseq file
    ...
    try:
        ...
        print("Exception occurred during opseq creation of apk ", apk_hash)
        logging.error('Exception occurred during opseq creation {0}'.format(str(e)))
        return False
...
def apktool_decode_apk(apk_file, out_file, include_libs):
    ...
    """
    C:/Windows/apktool -q d D:\5th\Honours\Code\dataset\Benign\androzoo/tmp/air.JAL.apk -s -r --no-assets --force-manifest
    """
    apktooldir = "C:/Windows"
    apktoolcmd = "{0}/apktool -q d -f {1} -o {2} -s -r --no-assets --force-manifest".format(apktooldir, apk_file, out_file)
