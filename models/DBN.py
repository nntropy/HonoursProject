# From https://github.com/fuzimaoxinan/Pytorch-Deep-Neural-Networks/
...
class DBN(nn.Module):
    ...
    def make_dataset(self, dataset, BATCH_SIZE=10):
        x, y = dataset[:, :166], dataset[:, -1:]
    ...
    def train_static(self, train_data, num_epochs=10, batch_size=20):
        '''
        Greedy Layer By Layer training
        Keeping previous layers as static
        '''

        train, test, y_test = self.make_dataset(dataset=train_data)
        train_data = train_data[:, :166]
        for i in range(len(self.rbm_layers)):
            print("-"*20)
            ...
            v = train_data.view((train_data.shape[0] , -1)).type(torch.FloatTensor)#flatten
        ...
        with torch.no_grad():
            for x_batch, y_batch in test:
                ...
                pre = self(x_batch)[1][0].tolist()
                y_test_pred = y_test_pred + pre
        ...
        torch.save(self.state_dict(), "D:\\5th\Honours\Code\models\weights/dbn2.pt")
        return
